...
namespace UnityPacker {
    public class Package : IEnumerable<KeyValuePair<string, OnDiskFile>> {
        ...
        public static OnDiskFile FileFromDisk(string packPath, string diskPath, bool respectMeta = true) {
            ...
            if (respectMeta && File.Exists(diskPath + ".meta")) {
                var metaFile = diskPath + ".meta";
                meta = ReadMeta(metaFile);
            }

            return new OnDiskFile(packPath, diskPath, meta);
        }
        ...
        public static Package FromDirectory(string dir, string packName, bool respectMeta, string[] omitExts,
            ...
            foreach (var file in files) {
                ...
                var f = FileFromDisk(file, file, respectMeta);
                pack.PushFile(f);
            }
